...
then
  ...
  export RUNDIR=${RUNDIR:=$SCRATCH/human-${THREADS}-${SLURM_JOB_ID}-$(date '+%Y%m%d_%H%M%S')}
...
#  export ILLUMINA_VERSION=18
  ...
  MPIRUN="srun -n" UPCRUN="upcrun -q -n" UPC_SHARED_HEAP_MB=${UPC_SHARED_HEAP_MB} ${INST}/bin/run_hipmer.sh $INST ${RUNDIR}/meraculous-human.config
