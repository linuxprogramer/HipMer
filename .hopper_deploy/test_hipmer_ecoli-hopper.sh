...
then
  ...
  export RUNDIR=${RUNDIR:=$SCRATCH/hipmer-test-hopper-ecoli-${THREADS}-${PBS_JOBID}-$(date '+%Y%m%d_%H%M%S')}
  mkdir $RUNDIR
  ...
  MPIRUN="aprun -n" UPCRUN="upcrun -n" UPC_SHARED_HEAP_MB=${UPC_SHARED_HEAP_MB} POST_RUN=check_ecoli_results.sh \
