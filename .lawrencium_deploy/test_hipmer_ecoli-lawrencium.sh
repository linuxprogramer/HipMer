...
then
  ...
  export RUNDIR=${RUNDIR:=$SCRATCH/hipmer-test-ecoli-${THREADS}-${SLURM_JOB_ID}-$(date '+%Y%m%d_%H%M%S')}
  echo "Preparing ${RUNDIR}"
  mkdir -p ${RUNDIR}
  ${INST}/bin/hipmer_setup_ecoli_data.sh ${INST} ${RUNDIR}
  
  UPC_SHARED_HEAP_MB=${UPC_SHARED_HEAP_MB:=1500}
  MPIRUN="mpirun -n" UPCRUN="upcrun -n" DEBUG=1 UPC_SHARED_HEAP_MB=${UPC_SHARED_HEAP_MB} POST_RUN=check_ecoli_results.sh \
