...
int MatchOpt(const char argument, const char *const options);
...
*         is no longer needed.
****************************************************************************/
option_t *GetOptList(const int argc, char *const argv[], const char *const options)
{
    int nextArg;
    option_t *head, *tail;
...
option_t *MakeOpt(const char option, char *const argument, const int index)
{
    ...
    opt = malloc_chk(sizeof(option_t));
