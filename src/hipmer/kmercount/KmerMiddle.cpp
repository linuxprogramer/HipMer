...
KmerMiddle::KmerMiddle(const char *s) {
  set_kmer(s);
}
...
// pre:  
// post: the DNA string in _km and is the same as in km 
KmerMiddle& KmerMiddle::operator=(const KmerMiddle& o) {
  if (this != &o) {
    for (size_t i = 0; i < MAX_K/32; i++) {
      longs[i] = o.longs[i];
    }
...
bool KmerMiddle::operator<(const KmerMiddle& o) const {
...
void KmerMiddle::set_kmer(const char *s)  {
  size_t i,j,l;
  memset(bytes,0,MAX_K/4);
...
uint64_t KmerMiddle::hash() const {
  uint64_t ret;
  assert(k_bytes>0);
...
KmerMiddle KmerMiddle::rep() const {
  KmerMiddle tw = twin();
...
KmerMiddle KmerMiddle::twin() const {
  KmerMiddle km(*this);
...
KmerMiddle KmerMiddle::getLink(const size_t index) const {
...
KmerMiddle KmerMiddle::forwardBase(const char b) const {
  KmerMiddle km(*this);
...
/********
  km.shiftBackward(2);
  assert(k_bytes>0);
  km.bytes[k_bytes-1] &= KmerMiddle::k_modmask;
...
bool KmerMiddle::equalUpToLastBase(const KmerMiddle & rhs)
{
	KmerMiddle Ashifted = rhs.backwardBase('A');
	KmerMiddle Bshifted = backwardBase('A');
...
KmerMiddle KmerMiddle::backwardBase(const char b) const {
  KmerMiddle km(*this);
...
std::string KmerMiddle::getBinary() const {
  ...
  /*
  assert(k_bytes>0);
  for (size_t i = 0; i < KmerMiddle::k_bytes; i++) {
...
// post: s[0,...,k-1] is the DNA string for the KmerMiddle km and s[k] = '\0'
void KmerMiddle::toString(char * s) const {
  size_t i,j,l;
...
std::string KmerMiddle::toString() const {
  char buf[MAX_K];
  toString(buf);
...
/*
void KmerMiddle::shiftForward(int shift) {
  ...
  if (shift>0) {
    if (shift < 8 ) {
      for (size_t i = KmerMiddle::k_bytes-1; i > 0; i--) {
        bytes[i] <<= shift;
...
/*
void KmerMiddle::shiftBackward(int shift) {
  if (shift > 0) {
    if (shift < 8) {
      assert(k_bytes>0); 
      for (size_t i = 0; i < KmerMiddle::k_bytes-1; i++) {
        bytes[i] >>= shift;
        bytes[i] |= (uint8_t) ( bytes[i+1] << (8-shift));
      }
      
      bytes[KmerMiddle::k_bytes-1] >>= shift;
    } else {
      assert(0); // bad
    }
  }
...
// post: The KmerMiddle size has been set to k
void KmerMiddle::set_k(unsigned int _k) {
  if(_k == k) {
...
unsigned int KmerMiddle::k = 0;
unsigned int KmerMiddle::k_bytes = 0;
//unsigned int KmerMiddle::k_longs = 0;
unsigned int KmerMiddle::k_modmask = 0;
