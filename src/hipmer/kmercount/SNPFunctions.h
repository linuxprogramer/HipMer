...
void MergeAll(vector< vector<kmerpack> > & Arrs2Merge, vector< pair<Kmer,int> > & MergedKMers)
{
	...
	{
		pair<MERARR, int> * heap = new pair<MERARR, int> [hsize];	// int -> source id
		...
		make_heap(heap, heap+hsize, greater< pair<MERARR, int > >());	// to ensure a min-heap
		MergedKMers.resize(mergedsize*2);	// twice as big for the twins
		twnind = mergedsize;

		while(hsize > 0)
		{
			pop_heap(heap, heap + hsize, greater< pair<MERARR, int> >());         // result is stored in heap[hsize-1]
			...
			if(curptr[source] != Arrs2Merge[source].size())	// That array has not been depleted
			{
				...
				push_heap(heap, heap+hsize, greater< pair<MERARR, int> >());
			}
			else
			{
				--hsize;
