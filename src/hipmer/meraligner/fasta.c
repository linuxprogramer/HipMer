...
OpenFASTA(char *seqfile)
{
  ...
  ffp = malloc_chk(sizeof(FASTAFILE));
...
ReadFASTA(FASTAFILE *ffp, char **ret_seq, char **ret_name, int *ret_L)
{
   ...
   */
  s  = strtok_r(ffp->buffer+1, " \t\n", &aux);
  name = malloc_chk(sizeof(char) * (strlen(s)+1));
  strcpy(name, s);
   ...
   * sequence length.
   */
  seq = malloc_chk(sizeof(char) * 128);     /* allocate seq in blocks of 128 residues */
  nalloc = 128;
  n = 0;
      ...
      for (s = ffp->buffer; *s != '\0'; s++)
	{
	  ...
	  if (nalloc == n)	        /* are we out of room in seq? if so, expand */
	    {			        /* (remember, need space for the final '\0')*/
	      nalloc += 128;
	      seq = realloc_chk(seq, sizeof(char) * nalloc);
	    }
	}
    }
  seq[n] = '\0';
