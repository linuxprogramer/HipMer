...
my %rejected = ();
my @rejectReasons = ("FORMAT","MINLEN","UNINFORMATIVE","TRUNCATED","SINGLETON");
...
#   $readStatus,$query,$qStart,$qStop,$qLength,$subject,$sStart,$sStop,$sLength,$strand
	...
	if (($thisStopStat eq "GAP") && ($nextStartStat eq "GAP")) {
	    ...
	    print "SINGLE\tSPLINT\t$thisContig.$thisContigEnd\t[@thisInfo]\t$nextContig.$nextContigEnd\t[@nextInfo]\n";
	    $nSplintsFound++;
	}
