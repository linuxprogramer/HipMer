...
then
    ...
    (mkdir -p ${HIPMER_HUMAN_DATA}.tmp; lfs setstripe -c 72 ${HIPMER_HUMAN_DATA}.tmp || /bin/true ; cd ${HIPMER_HUMAN_DATA}.tmp && curl --max-time 36000 -o - $HIPMER_HUMAN_DATA_URL | tar -xzf - && cd .. && mv ${HIPMER_HUMAN_DATA}.tmp ${HIPMER_HUMAN_DATA})
fi
